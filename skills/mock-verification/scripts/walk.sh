...
# servers into a 3Gi cgroup. Walks run one at a time (`aep` step 3, so a second
# live Chromium cannot OOM the pod), but the port is still searched rather than
# fixed: a server outlives the walk that started it, so 5173 may be held by the
# previous walk's or by something else on a laptop. A launch that dies on a bind
# collision moves to the next port by itself. A stale server from this app's
# earlier attempt is reaped before a new one starts.
