...
// The CLAUDE CODE classifier: what the run loop is told about each Agent SDK
// message (`MessageClass`, `runtime/port.ts`).
//
// The SDK's shapes (`type === "result"`, `subtype.startsWith("task_")`,
// `stream_event`, …) are read here, beside the translator that reads the same
// shapes, so the loop reads only a class (ADR-0012's amendment).
//
// It is also where the SDK's own HEALTH messages are read — the ones the
// translator drops. The translator answers "what did the agent do"; the
// `retry` and `stall_signal` classes answer "why is nothing happening" — without
// them the watchdog could say the model turn was the slow half, never why.
//
...
import { num, str } from "../fields.js";
import type { ApiRetryInfo, MessageClass, MessageClassifier, StallSignal } from "../port.js";

/** A name a notice prints; "unknown" rather than a blank where the runtime gave none. */
function named(v: unknown): string {
  return str(v) || "unknown";
}
  ...
  switch (m.subtype) {
    ...
    case "model_refusal_fallback": {
      ...
      return {
        ...
        code: "refusal",
        detail:
          `[model] ${named(m.original_model)} refused${category ? ` (${category})` : ""}` +
          ` — retried on ${named(m.fallback_model)}`,
      };
    }
    case "model_refusal_no_fallback": {
      ...
      return {
        ...
        code: "refusal",
        detail:
          `[model] ${named(m.original_model)} refused${category ? ` (${category})` : ""} and no fallback ran` +
          `${why ? `: ${why}` : ""}`,
      };
    }
    case "permission_denied": {
      ...
      return {
        ...
        detail: `[permission] ${named(m.tool_name)} denied${by ? ` by ${by}` : ""}${why ? `: ${why}` : ""}`,
      };
    }
    case "worker_shutting_down":
      // A snake_case reason set by the host CLI, never user input.
      return { level: "error", code: "terminated", detail: `[worker] shutting down: ${named(m.reason)}` };
    default:
      return undefined;
  }
}
 ...
 * counted as work.
 */
function isModelWaitFrame(m: Record<string, unknown>): boolean {
  return isStreamFrame(m) || (m.type === "system" && m.subtype === "thinking_tokens");
}
 ...
 * and 4,000 tokens.
 */
function isStreamFrame(m: Record<string, unknown>): boolean {
 ...
 * precisely the stall it exists to report, so it is its own class, routed round
 * `observe` exactly like a retry.
 *
 * Kept apart from `isModelWaitFrame` because only that one is the model:
 * `observeStream` records "the model is producing", and a tool that is merely
 * slow says nothing about the model at all.
 */
function isToolProgressFrame(m: Record<string, unknown>): boolean {
  return m.type === "tool_progress";
}
...
/**
 * A task lifecycle message — `task_started`, `task_progress`, `task_updated`,
 * `task_notification` — or the background roster. None of them is the lead
 * speaking, which is what the loop's input rule needs to know.
 *
 * `started` / `ended` say which task entered or left the running set:
 * `task_started` opens one, and `task_updated` with a terminal status or the
 * task's `task_notification` closes it — the two messages every settled task in
 * both recordings produces.
 *
 * `background_tasks_changed` is deliberately NOT read for ids, though it looks
 * like the authoritative list: it enumerates only BACKGROUNDED tasks, so a
 * foreground subagent (probe1's depth-2 child, `is_backgrounded: false`) would
 * be evicted by the next one that omits it and then never stopped.
 */
function taskBookkeeping(m: Record<string, unknown>): MessageClass | undefined {
  if (m.type !== "system") return undefined;
  const subtype = typeof m.subtype === "string" ? m.subtype : "";
  if (!subtype.startsWith("task_") && subtype !== "background_tasks_changed") return undefined;
  const taskId = typeof m.task_id === "string" ? m.task_id : "";
  if (!taskId) return { kind: "task_bookkeeping" };
  if (subtype === "task_started") return { kind: "task_bookkeeping", started: taskId };
  if (subtype === "task_notification") return { kind: "task_bookkeeping", ended: taskId };
  if (subtype === "task_updated") {
    ...
    if (TASK_TERMINAL.has(status)) return { kind: "task_bookkeeping", ended: taskId };
  }
  return { kind: "task_bookkeeping" };
}

/** The skills an `init` says the session resolved; a malformed list is none. */
function resolvedSkills(m: Record<string, unknown>): string[] {
  return Array.isArray(m.skills) ? (m.skills as string[]) : [];
}

/**
 * The classifier for ONE session.
 *
 * The order of the questions is load-bearing in exactly one place: a rate-limit
 * sentence the dedupe drops is not a stall signal, and falls through to
 * `activity` (translated to nothing, and observed). Every other class is
 * disjoint from the rest by shape, so no message is ever two of them.
 *
 * Per session because the rate-limit reader is: two runs sharing one would
 * swallow the second run's opening line.
 */
export function createClaudeClassifier(): MessageClassifier {
  const readStallSignal = createStallSignalReader();
  return (message) => {
    const retry = readApiRetry(message);
    if (retry) return { kind: "retry", info: retry };
    const signal = readStallSignal(message);
    if (signal) return { kind: "stall_signal", signal };
    if (!message || typeof message !== "object") return { kind: "activity" };
    const m = message as Record<string, unknown>;
    if (isModelWaitFrame(m)) return { kind: "model_wait", streaming: isStreamFrame(m) };
    if (isToolProgressFrame(m)) return { kind: "tool_progress" };
    if (m.type === "result") return { kind: "turn_end" };
    const bookkeeping = taskBookkeeping(m);
    if (bookkeeping) return bookkeeping;
    if (m.type === "system" && m.subtype === "init") return { kind: "init", resolvedSkills: resolvedSkills(m) };
