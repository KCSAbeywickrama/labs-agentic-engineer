...
- **Remote flow** — the coding-agent runner passes
  `plugins: [{ type: "local", path: <repo>/runners/remote-worker/plugin }]` to
  the Claude Agent SDK at dispatch.
- **Local flow** — a developer can install it into their own Claude Code:
  ```bash
  claude plugin install /path/to/asdlc/runners/remote-worker/plugin
  ```
