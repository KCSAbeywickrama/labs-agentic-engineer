...
//     BFF /api/v1/org/idp/discovery endpoint to populate JWKS URL from
...
export default function OrgIDPSettings() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const p = await orgIDPApi.getProfile();
      setProfile(p);
    } catch (err) {
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const updated = await orgIDPApi.updateProfile({
        kind: editKind,
        issuer: editIssuer,
        jwksUrl: editJWKS,
      });
  ...
  const handleRotate = useCallback(async () => {
    ...
    setRotating(true);
    try {
      const { clientSecret } = await orgIDPApi.rotateSecret();
