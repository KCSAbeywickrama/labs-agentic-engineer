 ...
 *
 * Reads projection via GET /api/v1/github (org from the verified JWT).
 * Renders one of:
...
export default function OrgGitHubSettings() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const proj = await orgGithubApi.getStatus();
      setProjection(proj);
    } catch (err) {
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      await orgGithubApi.disconnect(isApp ? uninstallOnDisconnect : false);
  ...
  return (
    <Stack gap={2}>
      ...
      {!isConnected && (
        <NotConnectedPanel onConnected={load} />
      )}
...
function NotConnectedPanel({ onConnected }: { onConnected: () => void }) {
  return (
    <Stack gap={2}>
