 ...
 * the paste field), or supply a publicly reachable URL. Submitting calls
 * `specsApi.collect` and on success shows the returned `specPath`, then calls
 * `onChanged()` so the parent can refresh the design query.
 *
 * The contract's `CollectSpecOutputBody` carries only `specPath` — unlike the
 * ported source, there is no `operationCount` field to surface.
 */
...
import { specsApi } from '../../services/api/specs';
...
export function ProvideSpec({
  ...
  const mutation = useMutation({
    mutationFn: () => {
      ...
      return specsApi.collect(projectId, component, dep.name, body);
    },
    onSuccess: () => {
      onChanged();
    },
  ...
  if (mutation.isSuccess && mutation.data) {
    return (
      <Box>
        <Alert severity="success">Spec attached ✓</Alert>
