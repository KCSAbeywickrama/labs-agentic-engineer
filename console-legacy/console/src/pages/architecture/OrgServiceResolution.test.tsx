...
describe('OrgServiceResolution', () => {
  ...
  describe('blocked / access-required — no prior request', () => {
    it('renders a warning and "Request access" button', async () => {
      vi.mocked(accessRequestsModule.accessRequestsApi.list).mockResolvedValue([]);
    ...
    it('calls accessRequestsApi.create with (projectId, component, dep.name) when clicked', async () => {
      vi.mocked(accessRequestsModule.accessRequestsApi.list).mockResolvedValue([]);
      vi.mocked(accessRequestsModule.accessRequestsApi.create).mockResolvedValue(mockAccessRequest);
      ...
      await waitFor(() => {
        expect(accessRequestsModule.accessRequestsApi.create).toHaveBeenCalledWith(
          'my-project',
          'api',
          'payment-svc',
        );
  ...
  describe('access-pending state with an in-flight request', () => {
    it('renders a status chip and a link to the provider issue', async () => {
      vi.mocked(accessRequestsModule.accessRequestsApi.list).mockResolvedValue([mockAccessRequest]);
