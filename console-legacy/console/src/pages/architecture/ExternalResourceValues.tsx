...
/**
 * ExternalResourceValues — per-environment value entry form for an `external`
 * dependency, embedded directly in the DependencyDrawer (no dialog wrapper).
 * Supports:
 ...
 *     values.
 *
 * Values are project-scoped (shared by every component in the project that
 * consumes this external resource) — `externalResourcesApi.saveValues` takes
 * no component name.
 */
...
interface ExternalResourceValuesProps {
  projectId: string;
  dep: Dependency;
  onSaved: () => void;
}
...
export function ExternalResourceValues({
  projectId,
  dep,
  onSaved,
}: ExternalResourceValuesProps): JSX.Element {
  ...
  const mutation = useMutation({
    mutationFn: () =>
      externalResourcesApi.saveValues(projectId, dep.name, {
        [selectedEnv]: values,
      }),
    onSuccess: () => {
      setSaved(true);
