 ...
 *                                 access-requests query cache and calls onChanged().
 ...
 *                                 providerIssueUrl when present. `access-pending` is
 *                                 CLIENT-DERIVED — reconciled here against in-flight
 *                                 AccessRequests fetched from accessRequestsApi.list.
...
import type { AccessRequest, Dependency } from '../../services/api/types';
import { accessRequestsApi } from '../../services/api/accessRequests';
import { ApiError } from '../../services/api/rest';

// ---------------------------------------------------------------------------
// Stable query-key helper — co-located here so OrgServiceResolution can
// invalidate its own cache after a successful "request access" mutation.
// ---------------------------------------------------------------------------

export const accessRequestsQueryKey = (projectId: string | undefined) => [
  'accessRequests',
  projectId,
];
...
export function OrgServiceResolution({
  ...
  const accessRequestsQuery = useQuery<AccessRequest[]>({
    ...
    queryFn: () => accessRequestsApi.list(projectId),
  });
  ...
  const mutation = useMutation({
    mutationFn: () => accessRequestsApi.create(projectId, component, dep.name),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: accessRequestsQueryKey(projectId) });
      onChanged();
    },
  });
