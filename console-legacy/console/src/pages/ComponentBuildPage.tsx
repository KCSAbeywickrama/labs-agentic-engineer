...
export default function ComponentBuildPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [c, b] = await Promise.all([
      ...
      api.listBuilds(projectId, componentId),
    ]);
    setComponent(c);
    setBuilds(b);
    setLoading(false);
  ...
  useEffect(() => {
    ...
    if (hasActive && projectId && componentId) {
      pollRef.current = setInterval(async () => {
        const updated = await api.listBuilds(projectId, componentId);
        setBuilds(updated);
      }, 5000);
    }
    return () => {
  ...
  const handleTriggerBuild = async () => {
    if (!projectId || !componentId) return;
    setTriggering(true);
    await api.triggerBuild(projectId, componentId);
    const updated = await api.listBuilds(projectId, componentId);
    setBuilds(updated);
    setTriggering(false);
  };
  ...
  const handleBuildClick = async (build: Build) => {
    ...
    setLogsLoading(true);
    try {
      const logs = await api.getBuildLogs(projectId, componentId, build.name);
      if (logs) {
        setBuildLogs(logs);
      } else {
