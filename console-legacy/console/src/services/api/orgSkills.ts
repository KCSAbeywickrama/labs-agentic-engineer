...
/**
 * Typed client for /api/v1/org/skills*. The active org is derived from the
...
export const orgSkillsApi = {
  ...
  async list(): Promise<SkillSummary[]> {
    const raw = await fetchJSON<{ skills?: SkillSummary[] } | SkillSummary[]>(orgSkills());
  ...
  async get(name: string): Promise<SkillDetail> {
    return fetchJSON<SkillDetail>(`${orgSkills()}/${encodeURIComponent(name)}`);
  },
  ...
  async create(
    ...
    return fetchJSON<SkillDetail>(orgSkills(), {
      method: 'POST',
  ...
  async remove(name: string): Promise<void> {
    await fetchJSON<void>(`${orgSkills()}/${encodeURIComponent(name)}`, {
      method: 'DELETE',
    });
  },
  ...
  async checkUpdates(): Promise<SkillUpdatesResponse> {
    const raw = await fetchJSON<SkillUpdatesResponse>(`${orgSkills()}/updates`);
  ...
  async importTarball(file: File): Promise<SkillImportResult> {
    const headers = await authHeader();
    const form = new FormData();
    form.append('file', file);
    const res = await fetch(`${BASE}${orgSkills()}/import`, {
      method: 'POST',
      headers,
      body: form,
    });
