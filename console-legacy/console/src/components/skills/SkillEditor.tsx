 ...
 */
export default function SkillEditor({ editName, open, onClose, onSaved }: SkillEditorProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEdit && editName) {
        await orgSkillsApi.update(editName, { skillMd });
        onSaved(editName);
      } else {
        const created = await orgSkillsApi.create({ name: name.trim(), skillMd });
