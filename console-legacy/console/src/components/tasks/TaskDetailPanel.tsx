...
export function TaskDetailPanel({ task, projectId, onClose }: TaskDetailPanelProps) {
  ...
  const handleRetry = async (e: React.MouseEvent) => {
    ...
    setIsRetrying(true);
    try {
      await api.retryTask(projectId, task.componentTaskId);
    } catch {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Retry surface for `failed` tasks — the coding-agent pod errored
            (new-path Job terminal Failed, e.g. missing env, image pull, OOM).
