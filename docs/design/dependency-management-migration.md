...
| 6 | **Provisioning-as-issues + funnel gates + declarative wiring** — real `aep:provision` adapters (the crux) | 🟢 green | ◐ CODE DONE (6.1–6.4 committed + unit/arch green + OpenAPI regen); live e2e pending |
...
- **Phase 6 (2026-07-06) ◐ CODE DONE — committed in 4 green sub-commits; live e2e pending.** Built the whole §3.6 model directly (subagents used only for the initial substrate mapping); each sub-commit is build/vet/test/arch green.
  ...
  - **Follow-ups carried out of Phase 6 (tracked, not abandoned):** (a) **orgPublished durability marker commit** — `GrantByProviderComponent` flips status + closes the issue but does NOT yet commit `exposesAPI.orgPublished` to the provider design; needs the single-file committed-truth write path (the same Phase-5 spec-commit deferral). The FUNCTIONAL org-service gate already works via Phase 5's live-catalog proceed-gate. (b) The two Phase-5 spec-commit deferrals (`StoreConsumedSpec` single-file commit + auto-fetch-on-save) remain — same write-path dependency. (c) reject auto-trigger on a declined org-publish is exposed (`RejectByProviderTask`) but not yet wired to a webhook signal.
  - **Remaining Phase-6 step:** live e2e on the OC 1.1.1 cluster — rebuild/redeploy aep-api, verify the provisioning routes are live (no 503), MCP discovery answers, and design-approve mints `aep:provision` gate issues. The FULL all-four-kinds build→deploy e2e is the Phase 8 gate (still blocked by the runner-image + `coding_agent_logs` follow-ups from Phase 1).
