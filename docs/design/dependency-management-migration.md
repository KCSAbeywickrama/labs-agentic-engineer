...
| 7 | Frontend: merge console-legacy dep UI + re-point CTAs to `aep:provision` | 🟢 green | ✅ **DONE + drawer e2e PASSED live** (`20b2c9a`) — DependencyDrawer mounted, held-task gate captions on our derived model, vitest 47/47, typecheck 0. Live Playwright (all 6 steps pass): dep rows + both panels + `?dep=` deep-link. Plus the **collect-dependency-spec route un-deferred** (`7e5b71c`) — the 4th drawer flow (ProvideSpec) now has its backend |
...
> **⏸️ Compaction checkpoints:** compacted at **Phase 5 → Phase 6** (before the crux) and again at **Phase 6 → Phase 7** (Phase 6 done + behavioral e2e passed live). **Currently resuming into Phase 7** — read the "▶ Resume after compaction" callout below (it is kept current for the next phase).
...
> ### ▶ Resume after compaction (read this first if you're a fresh context) — **NEXT: Phase 7 (frontend)**
> Executed **phase-by-phase, committed incrementally** onto branch **`rewrite-improve`** (working directly on it). **Phases 0–6 are ✅ DONE + committed** (`8f9d2ef` … `75c0641`); **Phase 6 passed a full behavioral e2e live** (fresh design → `dependencies[]` → 2 `aep:provision` gate issues → masked status, no secrets). **Phase 7 (console-legacy dependency UI + re-point CTAs) is next** — see §Phase 7 for the checklist. To continue:
> 1. **Where am I?** — §0 tracker + **§8 "Execution progress"** (per-phase commit SHAs + decisions). `git log --oneline -20`. Resume from the **first non-`✅ DONE`** row (Phase 7).
> 2. **Fixtures:** the **PR workspace** (read-only upstream tip `1076be7`, the parity oracle) is a worktree at **`../aep-pr-workspace`** — Phase 7 verifies the net-new console-legacy dep UI is byte-parity against it (`git diff HEAD ../aep-pr-workspace -- console-legacy/console/src/pages/architecture …`). The merge is long done (`upstream/aep-rewrite` is an ancestor of HEAD).
> 3. **Green gate** after a compact: `cd services/aep-api && go build ./... && go vet ./... && go test ./...` (dbtests SKIP w/o Docker = OK; **the 4 `agentfold` golden-parity tests are deliberately `t.Skip`'d** — see the golden-fixture-regen follow-up below, NOT a regression); `pnpm --filter @aep/agent-stream test` (43/43) + `pnpm turbo run typecheck` (green **except** `@aep/console`/apps-console — a tracked convergence deferral). The **cluster** is up (OC 1.1.1) — `docker ps` for the 7 `aep-*` services; aep-api + agents were rebuilt+redeployed 2026-07-06 with the Phase-6 + Bug-A/B fixes. Fresh teardown→setup→start→seed reproduces it (Colima 6–7cpu/8gb).
> 4. **Phase 7 shape** (§Phase 7 has the full checklist): the net-new dep UI (`services/api/{accessRequests,externalResources,provisioning,specs}.ts`, `pages/architecture/*`, `ExternalResourcesSettings.tsx`) + the `dependencies[]` type vocabulary came in at the merge/Phase 3 and typecheck; Phase 7 (a) resolves the remaining shared-file conflicts (`App.tsx` client/route registration, `ProjectArchitecturePage.tsx` mount `DependenciesSection`), (b) re-points the **task-coupled** CTAs (`types.ts` `Task` gate fields + `components/tasks/{TaskDetailPanel,TaskRow}.tsx`) onto our derived read model + `aep:provision` issue status, (c) **accept-theirs `apps/console`**, (d) **fixes the cell-diagram `buildProjectModel.ts`** to render `dependencies[]` (Phase-6 e2e found it doesn't yet). Re-point the FE API calls at the LIVE backend routes (the 7 provisioning ops are in the code-first `openapi.yaml`). Console-legacy **vitest** suites land here. Point the console at the running BFF (proxy prefix `/aep-api-service/api/v1`); the deployed console IS console-legacy.
> 5. **Live e2e** uses the **playwright-cli skill** (UI `http://localhost:8090`, admin/admin; deployed console = console-legacy). **Verify-fix-live discipline:** after any fix, redeploy + confirm it's live (migrations ran, per-org skills repo re-synced via `POST /org/skills/sync` — project-create does NOT reconcile versions; snapshots materialize lazily so check `git show HEAD:…SKILL.md` in the org-skills mirror, not the `snapshots/<sha>/` dir) BEFORE re-running the long e2e.
> 6. **In-depth review / large mechanical sweeps → subagents** (implementation = Opus/default; Sonnet only for test-heavy sweeps); verify their build/test claims before trusting.
>
> **Open follow-ups (tracked in §8, none abandoned):** (a) **golden-fixture regen** — the 4 `t.Skip`'d `agentfold` parity tests + `packages/agent-stream/test/seed.ts` still encode legacy `connections[]`; regenerate the recorded cassettes (`console-legacy/console/test/fixtures/turns/*.json.gz`, 6) + TS-fold goldens (`testdata/tsfold/*`) + `frontmatter_bundle.json`/`gen-frontmatter.mjs` + `seed.ts` to `dependencies[]`, then un-skip. The console-legacy `turnCassettes.test.ts` design-generate replay is also skipped for the same stale cassettes (the whole node:test cassette suite is now de-gated → `test:cassettes`). (b) **build→deploy leg** (runner-image repin + `coding_agent_logs` migration) — Phase 8. (c) The single-file **committed-truth write path now EXISTS and is proven** (`files.Apply`, used by `design.CollectSpec` — spec-collect route landed `7e5b71c`); the two remaining consumers of it are now unblocked, small fast-follows: **orgPublished durability marker commit** (grant path) + **auto-fetch-on-save** (fetch external `specUrl` hints at tag-cut). (d) **reject cascade** trigger (needs a decline signal). (e) builtin-skill `dependentApis` drift (pre-existing both branches).
...
- **⏸ Compaction checkpoint (2026-07-06, Phase 6 → Phase 7):** Phases 0–6 complete + committed (`8f9d2ef` … `75c0641`); **Phase 6 behavioral e2e passed live**. Paused here for context compaction. **Next: Phase 7** (frontend — merge the console-legacy dependency UI + re-point the task-coupled CTAs onto our `aep:provision`/derived read model + fix the cell-diagram `dependencies[]` rendering; accept-theirs `apps/console`). See the §0 "▶ Resume after compaction" callout (kept current for Phase 7) for the branch/fixtures/green-gate/e2e discipline + the full open-follow-ups list (golden-fixture regen, build→deploy leg, orgPublished marker, reject trigger). Green gate note: the 4 `agentfold` golden-parity tests are intentionally `t.Skip`'d (Bug-A fixture drift) — do NOT treat as a regression.
...
- **Phase 7 (2026-07-06) ◐ CODE DONE — committed `20b2c9a`; live drawer e2e pending.** The console-legacy half of the merge. Most of it was already in from earlier phases: the net-new dep UI (`services/api/{accessRequests,externalResources,provisioning,specs}.ts`, `pages/architecture/*`, `ExternalResourcesSettings.tsx`) is **byte-parity** with the workspace (`git diff HEAD ../aep-pr-workspace` empty) and typechecked since Phase 3; `App.tsx` already registered the four client token accessors + the `external-resources` route (only keep-ours diff = our `tasks/:taskId` route param); `buildProjectModel.ts` already renders `dependencies[]` edges byte-parity (the §0 "doesn't render yet" note was stale); `apps/console` is accept-theirs (already byte-parity). This commit wires the rest:
  - **DependencyDrawer mount** (`ProjectArchitecturePage.tsx`): the `DependenciesSection` was orphaned (imported only by its own drawer/test, never mounted). Mounted it + `DependencyDrawer` inside the "Component Design" view, added the `/architecture?dep=<name>` deep-link effect + a refetch-re-derive effect (open drawer tracks fresh read-time resolution; reference-guarded against loops). `onChanged={refreshBundle}`. **Per §3.6 item 10 the provisioning CTAs live in this drawer, not on the tasks board.**
  - **Held-task gate captions** — **no component change needed**: our kept-ours `TaskRow` already shows `Waiting for: {dependsOn.join(', ')}` for `pending`/`on_hold` off the GitHub-native **derived** read model (planning mints coding issues with `DependsOn=[<resource deps>]`, so provision-gate reasons surface there). Rewrote `TaskRow.test.tsx` onto `TaskView` (`derivedStatus` + flat `dependsOn`); **deleted the orphaned `TaskDetailPanel.test.tsx`** (that component was git-rm'd at the merge; its SYSTEM-task CTAs moved to the drawer, which has its own panel tests — ExternalResourceValues/PlatformResourcePanel/OrgServiceResolution/ProvideSpec).
  - **Test infra:** installed vitest 4 + testing-library + jsdom (matching upstream versions), scoped vitest `include` to `src/` (so it doesn't collect the `test/` node:test SSE-cassette suite), added `--experimental-require-module` (jsdom 29's html-encoding-sniffer ESM `require()`; no-op on Node ≥22.12 — our env is 22.11). **Component suite 47/47 green; console typecheck 0 errors.** No eslint gate for console-legacy (island, no config; not in root turbo/lint).
  - **Cassette suite de-gated (per user):** moved the node:test SSE-cassette suite out of the `test` gate into an opt-in `test:cassettes`; its `fixtures/turns/*.json.gz` recordings predate the `design.json`/`dependencies[]` schema (the golden-fixture-regen follow-up), so the design-generate replay fails the "authored design.json" assertion — also individually `skip`'d with a reason. This surfaced only now because the prior green-gate never ran `console-legacy pnpm test` (it isn't in turbo).
  - **Drawer e2e (2026-07-06, Sonnet Playwright, live cluster) — PASSED all 6 steps.** Rebuilt+redeployed console (verified live: fresh container, HTTP 200, bundle hash == the Phase-7 build). Against the Phase-6 project `payments-verify-run` (committed design: `orders-db` platform-resource + `stripe` external): login ✅ → Architecture ✅ → **Dependencies section, 2 rows** ✅ → `orders-db` row → **PlatformResourcePanel** (instances/storage/version + Provision) ✅ → `stripe` row → **ExternalResourceValues** (STRIPE_API_KEY/STRIPE_WEBHOOK_SECRET, "Save & provision") ✅ → **`?dep=stripe` deep-link auto-opens** ✅ → no feature JS/network errors. **The "unknown" status chips are NOT a gap:** our read-resolution (`artifact_store.go` + `design_json.go`) is byte-parity with upstream — upstream *also* computes no read-time status for external-without-`needsSpec` + platform-resource deps (shared baseline). Screenshots were throwaway (removed).
  - **collect-dependency-spec route un-deferred (2026-07-06, `7e5b71c`).** The e2e/user flagged that `specs.ts` POSTed `…/dependencies/{depName}/spec` with no backend. Turned out ~80% was already ported (`FetchSpecFromURL` SSRF / `ValidateOpenAPI` / `NormalizeOpenAPIYAML` / `ConsumedSpecPath`); only the commit was stubbed, and its blocker ("no committed-truth single-file write surface") was **stale** — `files.FilesService.Apply` (atomic apply → main, per-file baseSha CAS) exists. Implemented `design.CollectSpec`: resolve+validate the external target (404/400), fetch (SSRF) or accept, validate+normalize, then **one atomic 2-file commit** — the normalized spec at `…/dependencies/<dep>.openapi.yaml` + the component's `design.json` with `specPath` recorded (which clears the external-needs-spec proceed-gate; `specUrl` hint dropped; codec omits status/reason per ADR-0003). Arch held (design→artifacts only; the Files commit reaches design via a narrow `designFileCommitter` port adapted at the composition root — `internal/app/design_adapters.go`). Route + code-first openapi regen (#72 retained) + 7 unit tests green. **This is the Phase-5 spec-commit deferral (a) closed.** Now all 4 drawer resolution flows have a live backend. Remaining twin (b) **auto-fetch-on-save** is now unblocked (same commit path) but not yet wired — a small fast-follow.

- **Fast-follows + build→deploy unblock (2026-07-06, `580beef`).** Closed the now-unblocked follow-ups on the proven committed-truth write path + the pre-existing build→deploy blocker: **c1** auto-fetch-on-save (`SaveAndProceed` fetches external `specUrl` hints via `CollectSpec`, re-reads HEAD for the gate/tag); **c2** `design.MarkOrgPublished` durability marker committed on grant (narrow `OrgPublishMarker` port, setter-wired to break the design↔provisioning cycle); **d** reject cascade (webhook `Router` now CHAINS handlers per (event,action); `provisioning.OnIssueClosed` rejects ungranted riders on a declined org-publish close — the grant path flips riders to granted before it closes, so a platform close no-ops; `RepoLocator.ByFullName` added); **e** builtin `api-management`/`react-webapp` SKILL.md → `dependencies[]` (kind component/external) + `aep.version` 1→2; **b** `coding_agent_logs` migration re-keyed to `executions(id)` (the legacy `phase3` guarded on the dropped `component_tasks` → never created the table → JobWatcher 42P01) + runner image **v5** built & pushed to `docker.io/xlight05/aep-coding-agent-runner:v5`, config + manifests repinned v4→v5. Tests green (CollectSpec, router chain multi-handler + error-aggregation, OnIssueClosed reject/no-op); build/vet/gofmt/arch clean. `coding_agent_logs` verified live on a fresh boot (FK → executions(id)).
- **Phase 8 — file-verification pass (2026-07-06, 4-agent read-only cross-check) → `docs/design/dependency-management-file-verification.md`.** Full file-level audit of PR #85 (`1076be7`) vs HEAD (`580beef`). **Verdict: NO dependency-management capability silently dropped** — every relevant upstream file is TAKEN (byte-parity), ADAPTED (equivalent named + wired on our `aep:provision`/`depsGate` substrate, no stubs/dead code), or consciously SKIPPED, and all 9 intentional divergences match this §8 log. 7 gaps surfaced — all cleanup / docs / pre-existing / missing-tests, none a lost capability (tracked in "Deferred follow-ups" below as `[verify-N]`).
- **Phase 8 — fresh-cluster e2e attempt BLOCKED on bring-up (2026-07-06), not code.** The `teardown→setup→start→seed` fresh spawn came up with **only kube-system pods** — `setup-openchoreo.sh` never installed the OC control plane / Thunder / CNPG (`helm list -A` empty), so `seed-dev.sh` couldn't mint the seeder token and the happy-path e2e couldn't start. Torn down (incl. Colima) pending a user-directed re-spawn. **Next attempt:** run `setup.sh` with FULL output (not piped through `tail`) to catch where `helm upgrade --install openchoreo-control-plane` fails — likely a transient chart-pull / the OC 1.1.1 validating-webhook race the script already retries. The §6/Phase-8 quality-gate + happy-path-e2e (all four kinds → provision → dispatch-in-order → **build→deploy** → cross-project grant → delete-deprovision) + capability cross-check + merge finalization still owe a green run.
...
_Actionable gaps from the Phase-8 file-verification pass (`docs/design/dependency-management-file-verification.md`) — none a lost capability:_
- **[verify-1] Skill version-bump durability (highest signal).** The `go:generate` skill-vendoring may `rm -rf flow && cp` from a source that lacks the `aep.version` bump — risking a **silent revert** of the Bug-A `high-level-architecture` v2 bump AND the (e) `api-management`/`react-webapp` v2 bumps on the next `make build`. Confirm the source→target topology and put the bump on the true source of truth (+ a guard test).
- **[verify-2] `CreateProjectRequest.prompt` missing** from `openapi.yaml` + the Go Huma model (1 of 3 #72 fields; `search`/`nextCursor` survived). **PRE-dates #85** — lost at `429d5b5` (code-first regen hazard). Live impact: `apps/console` still sends it (dropped server-side) → fails `tsc`. Re-add to the Huma request model + regen the spec/clients.
- **[verify-3] Broken SKIP-leftover dead code in `services/agents`.** `evals/taskplanner/**` + `playground/{tasks,tasks.test}.ts` import deleted (SKIP'd) modules → fail `tsc -p tsconfig.playground.json` (invisible to `make typecheck`/`test`); stale doc `services/agents/design/task-planner-contract-parity.md`. Delete the orphans.
- **[verify-4] `CONTEXT.md`** missing the `## Dependency management` block (upstream has it; the vocabulary lives in `docs/glossary.md` here). Manual union.
- **[verify-5] `skills/planner/task-breakdown/SKILL.md` orphaned** on the Go `embed.go` (no `PlannerFS`); likely superseded by the org-skills-repo model — decide wire-or-delete + document.
- **[verify-6] Test-coverage drops (low):** `external_resource_repository_dbtest_test.go` dropped whole (CRUD coverage lost); no test exercises MCP-block attachment on the turn payload.
- **[verify-7] Stale `connections[]` eval fixture** (minor; the 7/7 eval tests still pass).

_Earlier-tracked:_
- **Golden-fixture regen** — the 4 `t.Skip`'d `agentfold` parity tests + `packages/agent-stream/test/seed.ts` + the console `turnCassettes` design-generate replay still encode legacy `connections[]`; regenerate the recorded cassettes + TS-fold goldens + `frontmatter_bundle.json`/`gen-frontmatter.mjs` to `dependencies[]`, then un-skip.
- **Build→deploy leg live-verify** — runner **v5** + the `coding_agent_logs` migration are in; exercise the full build→deploy tail of the happy path on the next fresh cluster (the Phase-1 blocker's fix).
- **`setup-openchoreo.sh` bring-up robustness** — the Phase-8 fresh spawn failed to install the OC control plane; capture full setup output next spawn and harden if it's not a one-off transient.
- **Console convergence onto `apps/console`** — both consoles coexist by design; converging is a tracked deferral.
