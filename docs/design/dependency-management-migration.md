...
| 7 | Frontend: merge console-legacy dep UI + re-point CTAs to `aep:provision` | 🟢 green | ◐ **CODE DONE + committed** (`20b2c9a`) — DependencyDrawer mounted, held-task gate captions confirmed on our derived model, vitest suite green (47/47), typecheck 0; live drawer e2e pending |
...
>
> **Open follow-ups (tracked in §8, none abandoned):** (a) **golden-fixture regen** — the 4 `t.Skip`'d `agentfold` parity tests + `packages/agent-stream/test/seed.ts` still encode legacy `connections[]`; regenerate the recorded cassettes (`console-legacy/console/test/fixtures/turns/*.json.gz`, 6) + TS-fold goldens (`testdata/tsfold/*`) + `frontmatter_bundle.json`/`gen-frontmatter.mjs` + `seed.ts` to `dependencies[]`, then un-skip. (b) **build→deploy leg** (runner-image repin + `coding_agent_logs` migration) — Phase 8. (c) **orgPublished durability marker commit** + the **spec-commit** deferrals — need the single-file committed-truth write path. (d) **reject cascade** trigger (needs a decline signal). (e) builtin-skill `dependentApis` drift (pre-existing both branches).
...
- **Phase 7 (2026-07-06) ◐ CODE DONE — committed `20b2c9a`; live drawer e2e pending.** The console-legacy half of the merge. Most of it was already in from earlier phases: the net-new dep UI (`services/api/{accessRequests,externalResources,provisioning,specs}.ts`, `pages/architecture/*`, `ExternalResourcesSettings.tsx`) is **byte-parity** with the workspace (`git diff HEAD ../aep-pr-workspace` empty) and typechecked since Phase 3; `App.tsx` already registered the four client token accessors + the `external-resources` route (only keep-ours diff = our `tasks/:taskId` route param); `buildProjectModel.ts` already renders `dependencies[]` edges byte-parity (the §0 "doesn't render yet" note was stale); `apps/console` is accept-theirs (already byte-parity). This commit wires the rest:
  ...
  - **Known FE→backend gap (tracked, not a Phase-7 bug):** `specs.ts` POSTs `…/dependencies/{depName}/spec`, which our backend does **not** serve — that's the Phase-5-deferred `StoreConsumedSpec` spec-collect route (needs the committed-truth single-file write path; §8 follow-up). So 3 of 4 drawer resolution flows are live (external values / platform-resource provision / org-service access); **spec-attach 404s** until the backend route lands. The `ProvideSpec` panel is byte-parity and ready.
