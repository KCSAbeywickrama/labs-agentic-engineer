...
describe("toSpecEntry", () => {
  it("keeps the full specs/ path and derives the group from the folder", () => {
    ...
    expect(
      toSpecEntry({ path: "specs/acceptance/bought-items.feature", sha: "c3" }),
    ).toEqual({ path: "specs/acceptance/bought-items.feature", sha: "c3", group: "validation" });
  });
  ...
  it("maps and drops hidden files in one pass", () => {
    expect(
      toSpecEntries([
        ...
        { path: "specs/acceptance/bought-items.feature", sha: "c3" },
      ]).map((e) => e.path),
    ).toEqual(["specs/requirements/prd.md", "specs/acceptance/bought-items.feature"]);
  });
  ...
  it("files the acceptance criteria under validation, not a fourth section", () => {
    expect(
      toSpecEntries([
        ...
        { path: "specs/acceptance/adding-items.feature", sha: "b" },
      ]).map((e) => e.group),
    ).toEqual(["validation", "validation"]);
  });

  // specs/validation/ held the retired criteria+e2e oracle and nothing else.
  // Legacy projects still have that JSON committed, so the folder has to stay
  // hidden — un-mapped it would reach the rail with no viewer left for it.
  // Dropped HERE because this one rule feeds both of the view's lists — the
  // committed files and the declared plan's entries — so the row, the ghost,
  // the section counter and the section's state all go with it.
  it("hides the retired specs/validation/ folder, whatever is left in it", () => {
    expect(
      toSpecEntries([
        { path: "specs/validation/validation-criteria.json", sha: "a" },
        { path: "specs/validation/plan.md", sha: "b" },
        { path: "specs/acceptance/bought-items.feature", sha: "c" },
      ]).map((e) => e.path),
    ).toEqual(["specs/acceptance/bought-items.feature"]);
