...
// The generation instruction for a one-shot signal.
//
// Design is derived from the current requirements. Requirements go through
// `/start`, sent VERBATIM: the server expands it and appends the idea captured
// at project creation, read from the project's own descriptor
...
// the interview that `withGrillingInterview` used to bolt on.
function instructionFor(signal: "requirements" | "design"): string {
  return signal === "design" ? buildDesignGenerationInstruction() : START_COMMAND;
}
...
export function AgentChatPanel({
  ...
  useEffect(() => {
    ...
    // `/start` IS the interview turn (the server enriches it with the stored
    // idea), so the grilling skill is known to apply — inline it up front and
    // the agents service skips the model's loadSkill round-trip (#335 latency).
    const instruction = instructionFor(autoGenerate);
    if (autoGenerate === "requirements") {
      sendRef.current(instruction, { eagerSkills: ["grilling"] });
    } else {
      sendRef.current(instruction);
    }
    onAutoGenerated?.();
  }, [autoGenerate, onAutoGenerated]);
  ...
  const submit = () => {
    ...
    //
    // `/start` is the exception: it goes through UNEXPANDED so the server can
    // recognize it and append the idea captured at project creation, which the
    // browser neither reads nor parses.
    send(parseStartCommand(draft) ? draft.trim() : (slashSkillInstruction(draft) ?? draft));
    setDraft("");
  };
