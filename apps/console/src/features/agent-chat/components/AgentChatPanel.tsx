...
export function AgentChatPanel({
  ...
  useEffect(() => {
    ...
    // Flow commands go VERBATIM: the server expands every /<skill> token and
    // decides the flow's eager skills (#373), so this CTA and a typed command
    // produce byte-identical turns.
  ...
  const submit = () => {
    ...
    // /start, /design): the SERVER expands every command token, enriches
    // /start with the captured idea, and decides the flow's eager skills
    // (#373). Unknown skills surface as the agent's loadSkill not-found
    // (client-side validation is parked, #325).
    send(draft.trim());
    setDraft("");
  };
