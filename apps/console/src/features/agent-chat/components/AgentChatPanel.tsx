...
export function AgentChatPanel({
  ...
  onClose: () => void;
  /** "Generate spec" CTA (#150): auto-send the first requirements turn once,
   *  seeded from the stored create prompt. */
  ...
  useEffect(() => {
    ...
    const prompt = readCreatePrompt(org, projectName);
    send(buildSpecGenerationInstruction(prompt));
    onAutoGenerated?.();
