...
describe("parseValidationReport", () => {
  ...
  it("carries failure/spec/healed/flaky/duration on the failing e2e row", () => {
    ...
    // generate-report.mjs writes failure as { message, location }. Reading it as
    // a bare string produced "" for every real report, so the view's failure
    // block was dead in production while string-shaped fixtures kept tests green.
  ...
  it("still accepts a string failure from an older report", () => {
    const legacy = JSON.stringify({
      schemaVersion: 1,
      criteria: [
        { id: "AC-1", method: "e2e", status: "fail", failure: "boom" },
      ],
    });
  ...
  it("drops a failure object carrying no message", () => {
