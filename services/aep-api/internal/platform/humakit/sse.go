...
// SSEBody builds a huma.StreamResponse body function that stamps the standard
// SSE response preamble — the four event-stream headers, an explicit 200, and
// an initial flush so the headers reach the client before the first frame —
// then hands the body writer + a per-chunk flush func to run. Shared by every
// SSE-streaming Huma operation (genai turns, task planning).
func SSEBody(run func(w io.Writer, flush func())) func(huma.Context) {
	return func(hctx huma.Context) {
...
func MapAgentsUpstreamError(err error, overrides map[int]error) (error, bool) {
	...
	return huma.Error502BadGateway("agents service error"), true
}
