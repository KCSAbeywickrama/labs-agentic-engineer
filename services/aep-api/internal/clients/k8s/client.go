...
func NewInClusterClient() (client.Client, error) {
	...
	// Bound every request. This client is reached from paths that hold a
	// per-project advisory lock (the deploy cascade) on a detached context —
	// an un-timeouted rest.Config would let one slow API-server call hold
	// that lock (and its Postgres connection) indefinitely. 30s matches the
	// house convention on the HTTP clients.
