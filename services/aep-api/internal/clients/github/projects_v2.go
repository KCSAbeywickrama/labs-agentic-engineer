...
// GitHub Projects v2 GraphQL surface — the gitrepo.BoardOps port. Folded onto
// the same *Client as the REST surface so REST-vs-GraphQL never leaks past the
// port; wiring holds one gitrepo.Host.
...
func (c *Client) graphqlRequest(ctx context.Context, pat, query string, variables map[string]any, dest any) error {
...
func (c *Client) GetOrgID(ctx context.Context, org, pat string) (string, error) {
...
func (c *Client) CreateGitHubV2Project(ctx context.Context, orgID, pat, title string) (string, error) {
...
func (c *Client) initProjectStatusOptions(ctx context.Context, projectID, pat string) error {
...
func (c *Client) GetProjectBoard(ctx context.Context, githubProjectID, pat string) (*gitrepo.ProjectBoardResult, error) {
	...
	result := &gitrepo.ProjectBoardResult{URL: data.Node.URL, Items: []gitrepo.BoardItem{}}
	...
	for _, node := range data.Node.Items.Nodes {
		...
		labels := make([]gitrepo.LabelInfo, 0, len(node.Content.Labels.Nodes))
		for _, l := range node.Content.Labels.Nodes {
			labels = append(labels, gitrepo.LabelInfo{Name: l.Name, Color: l.Color})
		}
		...
		result.Items = append(result.Items, gitrepo.BoardItem{
			ID:       node.ID,
...
func (c *Client) AddIssueToProject(ctx context.Context, githubProjectID, issueNodeID, pat string) error {
	addMutation := `
...
func (c *Client) MoveProjectItemToStatus(ctx context.Context, githubProjectID, issueURL, targetStatus, pat string) error {
	query := `
		query($projectId: ID!) {
			node(id: $projectId) {
				... on ProjectV2 {
...
func (c *Client) LinkProjectToRepository(ctx context.Context, githubProjectID, owner, repo, pat string) error {
