...
func TestComponentService_UpdateWorkflowEnvVars_Passthrough(t *testing.T) {
	...
	if err := NewComponentService(oc, nil, nil, nil, nil, nil, nil).UpdateWorkflowEnvVars(context.Background(), "acme", "web", "svc", []openchoreo.WorkflowEnvVarRef{{Key: "K", Value: "V"}}); err != nil {
