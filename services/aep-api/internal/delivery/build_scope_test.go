...
func TestServesStoriesStamp_RoundTrip(t *testing.T) {
	body := "## Scope\nDo the thing.\n"
	stamped := StampServesStories(body, []int{1, 2, 4})
	if got := ParseServesStories(stamped); !reflect.DeepEqual(got, []int{1, 2, 4}) {
		t.Fatalf("parse(stamp) = %v", got)
	}
	// Restamping replaces, never duplicates.
	restamped := StampServesStories(stamped, []int{7})
	if got := ParseServesStories(restamped); !reflect.DeepEqual(got, []int{7}) {
