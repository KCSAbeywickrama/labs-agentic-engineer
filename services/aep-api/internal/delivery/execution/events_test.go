...
func TestEvents_PRMerged_ReDeliveryAfterBuild_NoDuplicate(t *testing.T) {
	...
	_, b, _ := store.TryAdmit(context.Background(), &delivery.Execution{Repo: "o/r", IssueNumber: 2, Kind: string(taskmeta.KindBuild), Component: "order-service", CommitSHA: "abc123"})
...
func TestEvents_PRMerged_PathBasedFanOut(t *testing.T) {
	...
	exec := &fakeExecutor{store: store, startOK: true}
	prs := fakePRReader{
		states: map[int]*sourcecontrol.PullRequestState{7: {State: "closed", Merged: true, MergeCommitSHA: "sha1"}},
