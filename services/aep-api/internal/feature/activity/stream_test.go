...
func TestOpenStream_replaysExistingEvents(t *testing.T) {
	repo := &fakeRepo{}
	repo.insert(&models.ActivityEvent{
		ID: "a", Type: models.ActivityTypeSpecPublished, ActorKind: "user",
...
func TestOpenStream_tailsHubForNewEvents(t *testing.T) {
	...
	// Service.Record would insert. The loop's hub.Subscribe happens inside
	...
	repo.insert(&models.ActivityEvent{
		ID: "b", Type: models.ActivityTypeTaskStarted, ActorKind: "agent",
