...
// commit_design_file.go — a single-file commit primitive for `specs/design/`
// that lands a change on remote main directly — NO version tag. The tagged
// SaveDesign flow always mints the next `v<N>-<M>` revision; this path is for
// internal durability touch-ups (e.g. SetComponentOrgPublished persisting
...
// credential as saveDesignViaAPI (see save_via_api.go), but commits exactly
// one blob and stops short of the tag step.
...
// CommitDesignFile writes a single file under `specs/design/` directly to
// remote main via the GitHub Git Data API, without creating a version tag.
// `subPath` is relative to `specs/design/` (forward slashes; nested
// components allowed); `content` is the new file body; `message` is the
// commit message. Returns the new commit SHA, or ("", nil) when the file
// already matches main (no-op, no commit).
//
// Credentials are the org's service-identity (the same
// gitOps.Resolver().Resolve(orgID) + ResolveSaveIdentities path SaveDesign
// uses) — no user JWT is required, so this is safe to call from internal
// durability writers like SetComponentOrgPublished.
func (s *artifactService) CommitDesignFile(ctx context.Context, orgID, projectID, subPath, content, message string) (string, error) {
	...
	err = retryOnCASConflict(ctx, bucketKey, func() error {
		mainSHA, ferr := s.gitOps.GitData().GetRef(ctx, owner, repo, cred, "heads/"+repoRecord.DefaultBranch)
		if ferr != nil {
			return fmt.Errorf("get ref main: %w", ferr)
		}
		mainCommit, ferr := s.gitOps.GitData().GetCommit(ctx, owner, repo, cred, mainSHA)
		if ferr != nil {
			return fmt.Errorf("get commit %s: %w", mainSHA, ferr)
		}

		blobSHA, berr := s.gitOps.GitData().CreateBlob(ctx, owner, repo, cred, []byte(content))
		if berr != nil {
			return fmt.Errorf("create blob %s: %w", subPath, berr)
		}
		treeSHA, terr := s.gitOps.GitData().CreateTree(ctx, owner, repo, cred, mainCommit.TreeSHA, []gitrepo.TreeEntry{{
			Path: repoPath,
			Mode: "100644",
			Type: "blob",
			SHA:  blobSHA,
		...
		commitSHA, cerr := s.gitOps.GitData().CreateCommit(ctx, owner, repo, cred, gitrepo.CreateCommitRequest{
			Message:   message,
			TreeSHA:   treeSHA,
		...
		if uerr := s.gitOps.GitData().UpdateRef(ctx, owner, repo, cred, "heads/"+repoRecord.DefaultBranch, commitSHA, false); uerr != nil {
			return uerr
		}
