...
// endpoints.AccessService; wired at the composition root so the task feature
// needn't import that package. Optional — nil disables the step.
...
func (h *Handler) PullRequestClosed(ctx context.Context, event, action string, body []byte) error {
	...
	orgID, projectID, err := repositories.LookupOrgProjectByRepoURL(h.db.WithContext(ctx), p.Repository.FullName)
