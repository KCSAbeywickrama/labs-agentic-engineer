...
//	T3  POST /internal/v1/agents/tech-lead/plan; proxy data-plan-item to console.
//	T4  Persist N task rows; create GH issues in parallel (p-limit 3).
//	T5  POST /internal/v1/agents/tech-lead/detail; proxy body deltas; async edit GH.
...
func (s *taskService) proxyDetailStream(
	...
	for scanner.Scan() {
		...
		case "data-task-body-complete":
			...
			task.Body = d.Body
			// Scoped to the body column only — a full-row Save here would race
			// a concurrent dispatch (see UpdateBody doc).
			if err := s.taskRepo.UpdateBody(ctx, task.ID, d.Body); err != nil {
...
func (s *taskService) editIssueBodyWithRetries(ctx context.Context, task *models.ComponentTask, repoURL, repoSlug string) {
	...
	for attempt := 1; attempt <= 3; attempt++ {
		...
		if err == nil {
			// Scoped column write: this goroutine holds a task struct captured
			// at generation time (status=pending). A full-row Save would clobber
			// a concurrent dispatch's status/run_name/dispatched_at — the
			// last-writer-wins race that left dispatched tasks stuck pending.
			_ = s.taskRepo.SetBodySyncPending(ctx, task.ID, false)
			return
		}
		lastErr = err
	}
	_ = s.taskRepo.SetBodySyncPending(ctx, task.ID, true)
...
func buildPlanRequest(
	...
	for i, c := range components {
		// Populate the agents S2S wire field (still named DependsOn — a later
		// task owns that contract) from the unified component dependencies.
