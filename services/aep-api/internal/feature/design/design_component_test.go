...
// staticOrgResolver is a minimal artifacts.OrgServiceResolver: `visible` names
...
func TestDesignComponent_ListDependencies_ComputesStatusPerKind(t *testing.T) {
	...
	var got []apigen.ComponentDependencies
	...
	if d := byName["billing"]; d.Status != models.DependencyStatusResolved {
		t.Errorf("billing (org-service, visible): status = %q, want %q", d.Status, models.DependencyStatusResolved)
	}
	if d := byName["stripe"]; d.Status != models.DependencyStatusResolved || d.SpecPath != "dependencies/stripe.openapi.yaml" {
	...
	if d := byName["sendgrid"]; d.Status != models.DependencyStatusUnresolved || d.Reason != models.DependencyReasonNeedsInput {
	...
	if d := byName["orders-db"]; d.Status != models.DependencyStatusResolved || d.ResourceType != "postgres-cnpg" {
		t.Errorf("orders-db (platform-resource): status=%q resourceType=%q, want %q/postgres-cnpg",
			d.Status, d.ResourceType, models.DependencyStatusResolved)
	}
}
