...
- **`encoding` dies at the edge, in both directions.** The files handler decodes a write's content
  ...
  ~33%-larger base64 text. Undecodable base64 is a 400, and so is an encoding outside the enum —
  never a commit of garbage. Reads mirror it: a binary file (invalid UTF-8, or any NUL) answers base64
  with `FileContent.encoding` set — raw bytes in a JSON string would be silently corrupted
  (json.Marshal swaps invalid UTF-8 for U+FFFD); text answers exactly today's wire shape, encoding
  omitted.
