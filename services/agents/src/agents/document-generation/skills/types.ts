 ...
 *
 * Each skill is one file under `skills/` so the prompts are easy to review
 * and tweak without touching the route. Ported verbatim from
 * agents-legacy/src/skills/document-generation/ (see route.ts for the wire
 * contract this powers).
 */
export interface DocumentGenerationSkill {
  /** Stable identifier used in the URL path: /internal/v1/agents/document-generation/{id}. */
  id: string;
