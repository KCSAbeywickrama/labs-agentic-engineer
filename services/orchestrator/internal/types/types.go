...
// Package types holds the DTOs passed across the orchestrator's workflow and
// activity boundary. Shared here so neither workflows nor activities import the
// other (workflows invoke activities by registered name — determinism boundary,
// ADR-0004). Enum values come from the shared orchestration contract module.
package types
...
type GatePolicy struct {
	Requirements orchestration.GateMode
	Design       orchestration.GateMode
...
type DevelopmentFlowInput struct {
	...
	Project    string
	CycleID    string
	Source     orchestration.CycleSource
	StartPhase orchestration.Phase
