...
import { Dependency, SlimComponent } from "./schema.js";
  ...
  return {
    ...
    add_dependency: tool({
      description:
        ...
        "  - 'org-service': a service published by ANOTHER project in the org, declared by name.\n" +
        ...
        "  - 'platform-resource': a platform-provisioned resource (database / queue / cache / identity-provider), named by `resourceType`.\n" +
        "Idempotent on the dependency's `name` (a second add with the same name replaces it). NEVER include a `status` or `reason` key — resolution state is computed by the platform, not authored here.",
      inputSchema: z.object({
        name: z
          .string()
