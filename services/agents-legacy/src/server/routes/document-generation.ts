...
/**
 * Generic document-generation route. Path: `/internal/v1/agents/document-generation/{skillId}`.
 *
...
export function registerDocumentGeneration(app: express.Express) {
  app.post(
    `${AGENTS_BASE}/document-generation/:skillId`,
    async (req, res) => {
      ...
      const model: LanguageModel = createModel({ apiKey: getAnthropicKey(res) });
