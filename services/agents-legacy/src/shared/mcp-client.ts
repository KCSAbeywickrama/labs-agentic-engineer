 ...
 * optional there, so this client skips it and goes straight to `tools/list`
 * (one fewer round trip against a token minted with a short, ~5 min TTL).
 *
 * `loadMcpTools` DISCOVERS the server's tools via `tools/list` and wraps each as
 * an AI SDK `dynamicTool` whose execution proxies to `tools/call`. The
 * architect route (`server/routes/architect.ts`) merges these into the
 * architect's tool set (`run.ts`'s `extraTools`) so the design LLM can look up
 * the org's already-registered external resources / org endpoints / platform
 * resource types before proposing a `dependencies` entry — reusing an existing
 * name + schema instead of inventing one.
 *
 * PORT of lab-app-factory `agents/src/shared/mcp-client.ts`; adapted here (same
 * as `services/agents/src/shared/mcp-client.ts`) to carry a bearer token (the
 * source server had no auth, deriving the URL from an env var instead of the
 * wire) and to skip `initialize`.
 *
...
import type { McpConfig } from "../agents/architect/schema.js";
    ...
    for (const d of descriptors) {
      ...
      tools[d.name] = dynamicTool({
        ...
        execute: async (args) => {
          ...
          // tool-error convention: a thrown error inside a dynamic tool's
          // execute() is caught by the SDK and re-emitted as a tool-error
          // stream part, so the model sees a flagged failure instead of
          // misreading it as a successful lookup.
          if (result?.isError) {
