...
/**
 * Unit tests for the MCP client shim against a FAKE JSON-RPC http server (the
 * real one is aep-api's `internal/feature/dependencies/mcp_server.go`, not
 * reachable from these unit tests). Covers the failure modes the architect
 * route relies on being best-effort: a clean list/call round-trip (carrying
 * the bearer token, never calling `initialize`), 401 (expired/short-TTL
 * token), a malformed response, a malformed descriptor, an `isError:true`
 * tools/call result, and an unreachable server — none of them may throw.
 */
...
/** Wait for `server` (from `createServer().listen(0)`) to bind; return its URL + a close fn. */
async function listen0(server: Server): Promise<{ baseUrl: string; close: () => Promise<void> }> {
...
test("list/call round-trip: discovers tools/list and proxies tools/call, carrying the bearer token, never calling initialize", async () => {
  ...
  try {
    ...
    assert.ok(!seenMethods.includes("initialize"), `initialize should be skipped; saw: ${seenMethods.join(",")}`);
  } finally {
    await close();
  }
});
